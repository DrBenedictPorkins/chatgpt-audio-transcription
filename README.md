...
- Speaker diarization support (AssemblyAI) - identifies different speakers in conversations
- Advanced AssemblyAI options - custom spelling and expected speaker count
...
- `--output-file`: Specify the output file for transcriptions (default: [service]_raw_transcriptions.txt in the audio files directory)
...
- `--model`: Model to use for transcription (OpenAI only, default: whisper-1)

#### AssemblyAI-specific arguments:
...
The AssemblyAI transcription includes speaker labels (e.g., "Speaker 0: Hello, how are you?") to identify different people in conversations. The custom spelling option helps to correctly format terms, names, or technical jargon.
